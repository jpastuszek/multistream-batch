...
use crate::multi_buf_batch::{MultBufiBatch, PollResult};
...
/// Commands that can be send to `MultBufiBatchChannel` via `Sender` endpoint.
#[derive(Debug)]
...
/// 
#[derive(Debug)]
pub struct MultBufiBatchChannel<K: Debug + Ord + Hash, I: Debug> {
    channel: Receiver<Command<K, I>>,
    batch: MultBufiBatch<K, I>,
...
impl<K, I> MultBufiBatchChannel<K, I> where K: Debug + Ord + Hash + Send + Clone + 'static, I: Debug + Send + 'static {
    /// Creates new instance of `MultBufiBatchChannel` given maximum size of any single batch in number of items (`max_size`)
    ...
    /// and `MultBufiBatchChannel` receiver end which can be used to receive flushed batches.
    pub fn new(max_size: usize, max_duration: Duration, channel_size: usize) -> (Sender<Command<K, I>>, MultBufiBatchChannel<K, I>) {
        let (sender, receiver) = crossbeam_channel::bounded(channel_size);

        (sender, MultBufiBatchChannel {
            channel: receiver,
            batch: MultBufiBatch::new(max_size, max_duration),
            flush: None,
        })
    }

    /// Creates `MultBufiBatchChannel` with sender end embedded in newly spawned thread.
    /// 
    ...
    /// Returns `MultBufiBatchChannel` connected with the sender.
    pub fn with_producer_thread(max_size: usize, max_duration: Duration, channel_size: usize, producer: impl Fn(Sender<Command<K, I>>) -> () + Send + 'static) -> MultBufiBatchChannel<K, I> {
        let (sender, batch) = MultBufiBatchChannel::new(max_size, max_duration, channel_size);
...
mod tests {
    ...
    fn test_batch_max_size() {
        let (sender, mut batch) = MultBufiBatchChannel::new(4, Duration::from_secs(10), 20);
    ...
    fn test_batch_with_producer_thread() {
        let mut batch = MultBufiBatchChannel::with_producer_thread(2, Duration::from_secs(10), 20, |sender| {
            sender.send(Append(0, 1)).unwrap();
            sender.send(Append(1, 1)).unwrap();
            sender.send(Append(0, 2)).unwrap();
            sender.send(Append(1, 2)).unwrap();
    ...
    fn test_batch_max_duration() {
        let mut batch = MultBufiBatchChannel::with_producer_thread(2, Duration::from_millis(100) ,10, |sender| {
            sender.send(Append(0, 1)).unwrap();
    ...
    fn test_batch_disconnected() {
        let (sender, mut batch) = MultBufiBatchChannel::new(2, Duration::from_secs(10), 20);
